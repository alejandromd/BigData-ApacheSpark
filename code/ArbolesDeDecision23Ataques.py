...
spark = SparkSession.builder \
    .appName("ArbolesDeDecision23Ataques") \
    .getOrCreate()
...
attacks = data.map(lambda d: d[41]).distinct().collect()
print("Numero de ataques distintos: ", str(len(attacks)))
...
def process_line(line_split, protocols, services, flags):
    ...
    try:
        attack = attacks.index(line_split[41])
    except:
        attack = len(attacks)
        
    return LabeledPoint(float(attack), array([float(x) for x in new_line]))
...
t0 = time()
tree_model = DecisionTree.trainClassifier(training_data, numClasses=len(attacks), categoricalFeaturesInfo={1: len(protocols), 2: len(services),3: len(flags)},
